...
export async function getClientDossier(accountId) {
  await connectDb();
  const client = await Client.findOne({ accountId, deletedAt: null }).lean();
...
export async function getProfessionalWorkspace(account) {
  ...
  const clients = await Client.find({ professionalId: String(professional._id), deletedAt: null }).sort({ updatedAt: -1 }).lean();
...
export async function getSalonWorkspace(account) {
  ...
  const clients = await Client.find({ salonId: String(salon._id), deletedAt: null }).sort({ updatedAt: -1 }).lean();
