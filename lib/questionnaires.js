...
const sourceDefinitionIds = {
  ...
  fitzpatrick: "fitzpatrick-screening",
  dermatita: "dermatitis-screening",
  psoriazis: "psoriasis-screening",
  acnee: "acne-severity",
  hiperpigmentare: "hyperpigmentation-observation",
  eritem: "erythema-vascular-observation",
  pori: "pores-comedones-observation",
  ochi: "eye-area-observation",
  elasticitate: "elasticity-wrinkles-observation"
};

export function getAssessmentDefinition(id) {
  return id === "baumann" ? null : questionnaireDefinitions[sourceDefinitionIds[id]] || null;
}

export function evaluateAssessment(definition, answers = {}) {
  const missingQuestionIds = definition.questions.filter((question) => !String(answers[question.id] || "").trim()).map((question) => question.id);
  if (missingQuestionIds.length) return { ok: false, missingQuestionIds };
  if (definition.kind === "professional-observation") return { ok: true, score: null, label: `${definition.title} consemnată` };
  const score = definition.questions.reduce((total, question) => total + (question.options.find((option) => option.value === answers[question.id])?.points ?? 0), 0);
  const band = definition.bands.find((item) => score >= item.min && score <= item.max);
  return { ok: true, score, label: band?.label || "Evaluare înregistrată", summary: band?.summary || "Răspunsurile au fost salvate în dosar." };
}
...
import { questionnaireDefinitions } from "./source-questionnaires";
