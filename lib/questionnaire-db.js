...
function staticTemplateToDb(catalogEntry) {
  ...
  return {
    ...
    group: catalogEntry.group || "Chestionare",
...
export async function getEvaluableDefinition(slug) {
  ...
  return {
    ...
    group: template.group || questionnaireCatalog.find((entry) => entry.slug === template.slug)?.group || "Chestionare",
...
export async function listActiveEvaluableDefinitions() {
  ...
  return templates
    ...
    .filter((template) => template.questions.length > 0 || (template.dimensions && Object.keys(template.dimensions).length > 0));
}
  ...
  return templates
    ...
    .map((t) => {
      ...
      return {
        ...
        group: t.group || questionnaireCatalog.find((entry) => entry.slug === t.slug)?.group || "Chestionare",
