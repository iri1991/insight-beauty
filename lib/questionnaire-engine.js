...
function evaluateBaumannChoiceQuestionnaire(definition, answers = {}) {
  const details = definition.questions.map((question) => {
    const selectedValue = answers[question.id];
    const selectedOption = question.options.find((option) => option.value === selectedValue);
    return selectedOption
      ? { questionId: question.id, axis: question.axis, selectedValue, selectedLabel: selectedOption.label, points: selectedOption.points, missing: false }
      : { questionId: question.id, axis: question.axis, missing: true };
  });
  const missingQuestionIds = details.filter((detail) => detail.missing).map((detail) => detail.questionId);
  if (missingQuestionIds.length > 0) return { ok: false, missingQuestionIds };

  const pairs = { "O/D": ["O", "D"], "S/R": ["R", "S"], "P/N": ["N", "P"], "W/T": ["T", "W"] };
  const dimensions = details.map((detail) => {
    const code = detail.points <= 2.5 ? pairs[detail.axis][0] : pairs[detail.axis][1];
    return { id: detail.questionId, label: detail.axis, score: detail.points, code, band: { label: detail.selectedLabel, summary: `Răspuns înregistrat pentru axa ${detail.axis}.` } };
  });
  const code = dimensions.map((dimension) => dimension.code).join("");
  const tipologyDocument = getTipologyByCode(code);
  return {
    ...
    band: {
      label: `Tipologie Baumann ${code}`,
      summary: tipologyDocument?.preview || "Tipologie calculată din cele patru axe ale evaluării Baumann.",
      recommendation: "Rezultatul se discută cu profesionistul în contextul istoricului, observațiilor și planului de îngrijire."
    },
    tipologyDocument
  };
}

function evaluateProfessionalObservation(definition, answers = {}) {
  ...
  return {
    ok: true,
    details: definition.questions.map((question) => ({ questionId: question.id, label: question.label, value: String(answers[question.id]).trim() })),
    band: {
      label: `${definition.title} — observații consemnate`,
      summary: "Observațiile vizuale au fost salvate în fișa clientului pentru comparație la următoarea vizită.",
      recommendation: "Interpretarea și protocolul se stabilesc de profesionist după evaluarea completă."
    }
  };
}
...
export function evaluateDefinition(definition, payload = {}) {
  ...
  if (definition.kind === "baumann-choice") {
    return evaluateBaumannChoiceQuestionnaire(definition, payload.answers);
  }

  if (definition.kind === "professional-observation") {
    return evaluateProfessionalObservation(definition, payload.answers);
  }
